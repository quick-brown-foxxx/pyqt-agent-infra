...
- `src/qt_ai_dev_tools/` — Python package: AT-SPI tree traversal, xdotool interaction, CLI
  ...
  - `_env.py` — Centralized env var registry: `EnvVar` descriptors, `get_bool()`/`get_str()` accessors
  - `_vm_tool.py` — VM tool readiness check: version match (PyPI), source hash staleness (install-and-own)
...
- **VM provisioning** — qt-ai-dev-tools is installed in the VM via `uv tool install` with a pinned version. If a local install-and-own copy exists at `/vagrant/.qt-ai-dev-tools/`, provisioning runs `uv tool install --force` from that directory; otherwise it installs from PyPI. The tool binary lands in `~/.local/bin/` and the tool venv lives at `~/.local/share/uv/tools/qt-ai-dev-tools/`. System gi/pygobject packages are symlinked into the tool venv for AT-SPI access. The install-and-own copy includes a generated `pyproject.toml` for `uv tool install` compatibility. The provisioning template does not run `uv sync` on the user's project. Use the `provision-dev` make target for development reprovisioning.
...
- **Env var registry** (`_env.py`) — All environment variables the tool reads are defined as `EnvVar` descriptors in `_env.py`. Use `get_bool()` / `get_str()` instead of raw `os.environ.get()`. Registered vars: `QT_AI_DEV_TOOLS_VM`, `QT_AI_DEV_TOOLS_BRIDGE`, `QT_AI_DEV_TOOLS_ALLOW_VERSION_MISMATCH`, `DISPLAY`.
- **Tool readiness check** (`_vm_tool.py`) — Before proxying commands to the VM, the CLI calls `ensure_tool_ready()`. PyPI mode: checks version match (host vs VM), errors on mismatch (suppressed by `QT_AI_DEV_TOOLS_ALLOW_VERSION_MISMATCH=1`). Install-and-own mode: checks source hash staleness, auto-rebuilds via `uv tool install --force` and re-links gi.
